...
Если убрать `_`, а диапазоны не покроют все значения `i8`, компилятор не намекнёт, а выйдет в окно с `patterns `i8::MIN..=-11_i8` and `16_i8..=i8::MAX` not covered`, так как match не исчерпывающий. 